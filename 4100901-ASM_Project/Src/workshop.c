...
typedef struct {
    volatile uint32_t IMR1;    // Máscara de interrupción
    volatile uint32_t EMR1;    // Máscara de eventos
    volatile uint32_t RTSR1;   // Configuración de flanco ascendente
    volatile uint32_t FTSR1;   // Configuración de flanco descendente
    volatile uint32_t SWIER1;  // Generación de eventos por software
    volatile uint32_t PR1;     // Pendiente de interrupción
} EXTI_TypeDef;

typedef struct {
    volatile uint32_t EXTICR[4]; // Configuración de interrupción externa
    volatile uint32_t RESERVED;   // Reservado
    volatile uint32_t CMPCR;      // Compensación de temperatura
} SYSCFG_TypeDef;
...
#define EXTI ((EXTI_TypeDef *)0x40010400U) // Dirección base de EXTI

#define SYSCFG ((SYSCFG_TypeDef *)0x40013800U) // Dirección base de SYSCFG
...
{
    RCC->APB2ENR |= (1 << 0);            // Habilita el reloj de SYSCFG
    SYSCFG->EXTICR[3] &= ~(0x0F << 4);   // Limpia bits
    SYSCFG->EXTICR[3] |=  (0x02 << 4);   // Configura PC13 como fuente de interrupción
    EXTI->IMR1 |= (1 << B1_PIN);         // Habilita la interrupción para PC13
    EXTI->FTSR1 |= (1 << B1_PIN);        // Habilita la interrupción por flanco de bajada
...
// --- Manejador de la interrupción EXTI15_10 ----------------------------------
